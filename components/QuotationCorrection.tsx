...
export default function QuotationCorrection({ q }: { q: QuotationRow }) {
  ...
  return (
    <div className="mt-4 rounded-lg border border-neutral-200 bg-neutral-50/60 px-3 py-3">
      ...
      {codes.length > 0 && (
        <div className="mt-3 space-y-1.5">
          <p className="text-[11px] text-neutral-400">Afwijkende inkoopprijs per vloerregel (€/m²)</p>
          {codes.map(({ code, base }) => (
            <div key={code} className="flex items-center gap-2">
              <span className="w-40 shrink-0 truncate rounded bg-neutral-100 px-1.5 py-0.5 text-xs font-medium text-neutral-700">
                {formatProduct(code)}
              </span>
              ...
              <input
                ...
                disabled={busy}
                className="w-24 rounded-md border border-neutral-300 bg-white px-1.5 py-0.5 text-xs outline-none focus:border-emerald-500"
              />
              <button
                type="button"
              ...
              {(prices[code] ?? '') !== '' && (
                <button
                  ...
                  disabled={busy}
                  className="text-xs text-neutral-400 hover:text-rose-600 disabled:opacity-40"
